...
- [x] 실제 원천 데이터 재수집 — 12종 완료 (2026-06-07, 원본 컬럼 보존 확인)
  - 방범CCTV 12,168 → **21,060행**으로 증가 + 장비종류·시설명칭 컬럼 추가, 교차로는 지번·코드 포함 12컬럼
  - 어린이보호구역 CCTV도 원본(시설명·상세주소 포함, 41행)으로 교체 확인
...
- [ ] **(다음 작업) zip 내용물 검증**: SHP 구성 파일·좌표계(EPSG)·속성 컬럼·부산 범위 확인, D162/D164 정체 확인, 2시점 중 사용본 결정
- [ ] **(다음 작업) API 키 검증**: `.env`의 DATA_KEY로 대피장소 표준데이터 API 호출 확인 + 기상청 단기예보 등 나머지 필수 API 활용신청 여부 점검
- [ ] **(다음 작업) 새 원천 데이터 구조 분석**: CSV 12종 + SHP 컬럼/타입/결측/좌표 형식 파악, 보존할 컬럼 결정
