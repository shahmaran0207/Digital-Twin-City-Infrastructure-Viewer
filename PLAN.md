...
# Phase 1. 데이터 기반 구축 🔄 재작업 예정 (1-R 참고)
...
## 1-R. 데이터 재구축 — 원천 데이터 교체 ⬜ (2026-06-07 결정)
...
- [x] 실제 원천 데이터 재수집 — 11종 완료 (2026-06-07, 원본 컬럼 보존 확인)
  - 방범CCTV 12,168 → **21,060행**으로 증가 + 장비종류·시설명칭 컬럼 추가, 교차로는 지번·코드 포함 12컬럼
  - [ ] 잔여 1종: 어린이보호구역 내 불법주정차 CCTV 원본 (또는 전국무인교통단속카메라 표준데이터로 대체 결정)
...
- [ ] 추가 데이터 확보 (SOURCES.md D절 우선순위 기준): 시군구 경계 → 건물·DEM → 노드링크 → API 키 발급
...
- [ ] **테이블 재설계**: 새 데이터 컬럼 기준으로 `facility` 스키마 재검토
  ...
  - **시뮬레이션 입력 데이터(건물·DEM·노드링크)는 별도 테이블로 설계** — 채택 시뮬레이션 확정([plans/simulation.md](plans/simulation.md)) 후 반영
  - 마이그레이션 재작성 (`V1__init_schema.sql` 개정 또는 신규 버전)
...
> 후보 5종 조사 완료 (2026-06-07) — **채택 범위는 사용자 결정 필요** (A안: 그림자+침수+CCTV커버리지 / B안: +교통우회 / C안: +대피)

## 5-1. 범위 확정 + 데이터 준비
- [ ] 채택안 결정 (A/B/C) — 1-R 테이블 재설계 전에 확정 (스키마 반영 필요)
- [ ] 시뮬레이션 입력 데이터 확보·적재: DEM(침수), 건물(그림자·차폐), 노드링크(교통·대피, B안 이상)

## 5-2. 구현 후보 (채택안에 따라 선별)
...
- [ ] **교통 우회 (B안)**: pgRouting `pgr_dijkstra`, 링크 차단 전/후 경로 비교 (난이도 중)
- [ ] **대피 (C안)**: 격자인구 → 최근접 대피장소 경로/도달시간 (난이도 중~상)
...
| 3 | Phase 3-3~3-4 → 4 → 5(시뮬레이션) → 6 → 7 | 순차 진행, 각 착수 시 세부 계획 md 작성 |
